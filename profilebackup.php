...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    // Use 'phone' as the column name based on the uploaded image
    $new_phone = preg_replace('/[^0-9\-\+]/', '', filter_input(INPUT_POST, 'phone', FILTER_SANITIZE_STRING)); 

    // Prepare the UPDATE statement using the user_id (the session key)
    // ✅ Updated table fields from email, mobile_number to email, phone
    ...
    if ($stmt === false) {
        ...
        // 'ssi' -> two strings (email, phone), one integer (user_id)
        // If your user_id is a string (e.g., UUID), change 'i' to 's'
        // NOTE: If 'user_id' is VARCHAR(50) as per your image, the type should be 's'
        $stmt->bind_param("ssi", $new_email, $new_phone, $user_id);
        
        if ($stmt->execute()) {
            // Update session variables to reflect changes (good practice for immediate use)
...
// ----------------------------------------------------------------
// DATABASE LOOKUP: Fetching data for the logged-in user for display
// ----------------------------------------------------------------
...
$email = 'N/A';
$mobile_number = 'N/A'; // Renamed from mobile_number to phone in database structure, but using this variable for input field compatibility
$meter_number = 'N/A'; // Added meter_number
// Placeholder icon (or you can use a real image path if available)
$profile_image = 'https://placehold.co/80x80/3498db/ffffff?text=P'; 

// ✅ Updated SELECT statement to include all new columns from your database structure
$stmt = $conn->prepare("SELECT first_name, middle_name, last_name, extension, meter_number, email, phone FROM users WHERE user_id = ?");

if ($stmt === false) {
    $message = "Error fetching profile data: " . $conn->error;
    $message_type = 'error';
} else {
    // Assuming 'i' for integer type for user_id (adjust to 's' if VARCHAR)
    ...
    if ($result->num_rows > 0) {
        ...
        // Populate display variables with fresh data from the DB
        $first_name = $user_data['first_name'];
        $middle_name = $user_data['middle_name'] ?? ''; // Can be NULL
        $last_name = $user_data['last_name'];
        $extension = $user_data['extension'] ?? ''; // Can be NULL
        $meter_number = $user_data['meter_number'];
        $email = $user_data['email'];
        $mobile_number = $user_data['phone']; // Fetched from 'phone' column
...
// This address is now fixed as requested
...
// We need to change the $profile_image variable used in HTML to $profile_image_url for consistency
...
<head>
  ...
  <style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .nav-links a { font-size:14px; font-weight:600; color:var(--navy); text-decoration:none; transition:color 0.2s; }
    .nav-links a:hover { color:var(--blue); }
    .hamburger { font-size:21px; color:var(--navy); cursor:pointer; padding:4px; border-radius:50%; }
    ...
    .btn-cancel { background:var(--red); color:white; border:none; padding:11px 24px; border-radius:10px; cursor:pointer; font-weight:600; }
    .btn-action { background:var(--blue); color:white; border:none; padding:11px 32px; border-radius:10px; cursor:pointer; font-weight:600; }
...
<body>
  ...
  <!-- MAIN CONTENT - YOUR FULL PROFILE PAGE -->
  <div class="wrapper">
  ...
  <div class="modal-overlay" id="passwordModal">
    <div class="modal">
      ...
      <input type="password" class="modal-input" placeholder="Current Password" id="currentPass">
      <input type="password" class="modal-input" placeholder="New Password" id="newPass">
      <input type="password" class="modal-input" placeholder="Confirm New Password" id="confirmPass">
  ...
  <!-- FINAL PERFECT FOOTER - 100% MATCH TO YOUR DESIGN -->
  ...
  <script>
    ...
    new Chart(ctx, {
      ...
      options: {
        ...
        plugins: { title: { display: true, text: 'Monthly Consumption', font: { size: 20, weight: 'bold' }, color: '#08245c' }},
