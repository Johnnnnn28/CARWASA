...
// --- 5. DATA RETRIEVAL LOGIC: Fetch Water Interruption Notices ---

// Select all columns from the 'water_interruption' table
// Order by the date/time of the interruption in descending order (newest first)
$sql = "SELECT * FROM water_interruptions ORDER BY day ASC";
$result = $conn->query($sql);

// Initialize an empty array to store the fetched notices
$interruption_notices = [];

if ($result && $result->num_rows > 0) {
    // Loop through the results and store each row in the array
    while($row = $result->fetch_assoc()) {
        $interruption_notices[] = $row;
    }
}

// --- 6. CATEGORIZE NOTICES FOR DISPLAY ---
$emergency_notices = [];
$scheduled_notices = [];

foreach ($interruption_notices as $notice) {
    // Use strtoupper() and trim() for robust matching against 'EMERGENCY' or 'SCHEDULED'
    $type = strtoupper(trim($notice['type'])); 

    if ($type === 'EMERGENCY') {
        $emergency_notices[] = $notice;
    } elseif ($type === 'SCHEDULED') {
        $scheduled_notices[] = $notice;
    }
    // Notices with any other type are simply ignored for this display.
}
...
// Close the database connection (optional, as PHP closes it automatically at script end)
$conn->close();

// Now, the $interruption_notices array holds all your data:
// e.g., $interruption_notices[0]['title'], $interruption_notices[0]['description'], etc.
...
<head>
    ...
    <title>CARWASA - Water Interruption Advisory</title>
    ...
    <style>
        :root {
            ...
            --color-bg: #dbeaf0; 
            --color-alert: #ff6b6b; 
            --color-scheduled: #45cade; /* Added color for scheduled */
        ...
        *, *::before, *::after { 
        ...
        .main-nav ul {
            display: flex;
            align-items: center; /* Align items with the new menu image */
            gap: 40px;
        }
        ...
        /* New Menu Image Styling */
        .nav-menu-image {
            width: 32px; /* Set desired size for the menu icon */
            height: 32px;
            object-fit: contain;
        ...
        .nav-menu-image-link {
            padding: 5px; /* Adds clickable area padding */
            display: block;
        }
        ...
        */
        ...
        /* Water Interruption Specific Styles */
        ...
        /* Container for the lists (Emergency or Scheduled) */
        .advisory-list {
            margin-bottom: 40px;
        }
        ...
        .advisory-card {
            ...
            padding: 40px;
            margin-bottom: 40px;
            border-left: 8px solid var(--color-alert); /* Default for Emergency */
        }
        
        .advisory-card.scheduled {
        ...
        .advisory-title {
            font-size: 32px;
            font-weight: 800;
            color: var(--color-alert); /* Default for Emergency */
            margin-bottom: 20px;
            font-family: var(--font-heading);
        }
        
        .advisory-title.scheduled {
            color: var(--color-scheduled);
        }
        
        /* New style for the list of interruptions */
        .advisory-list-item {
        ...
        .advisory-list-item:hover {
            background-color: #3aa8bf; /* Slightly darker shade of secondary */
        }

        .advisory-list-item strong {
            font-weight: 700;
            flex-shrink: 0;
        }
        
        .advisory-list-item.emergency {
            background-color: var(--color-alert);
        }

        .advisory-list-item.emergency:hover {
            background-color: #e55c5c; /* Slightly darker shade of alert */
        }

        /* Existing styles below this line are kept for structure, but are commented out as they are no longer used for the list view requested. */
        ...
        @media (max-width: 992px) {
            /* Hide the image menu and show the burger menu on smaller screens */
            ...
            .advisory-header { font-size: 38px; }
        ...
        @media (max-width: 768px) {
            ...
            .advisory-header { font-size: 32px; }
            .footer-grid { 
...
<body>
    ...
    <div class="mobile-menu" id="mobileMenu">
        ...
        <div class="mobile-nav-links">
            ...
            <a href="pay-bill.html">Pay Your Bill</a>
            <a href="services.php">Services</a>
            <a href="news.html" class="active-link-style">News & Advisories</a>
            <a href="aboutusHOME.php">About</a>
        </div>
    </div>
    ...
    <div class="main-container">
        ...
        <section class="page-hero">
            <h1 class="banner-title">ADVISORIES</h1>
        ...
        <div class="content-wrap">
            <div class="content-area">
                ...
                <div class="advisory-header">
                    <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: var(--color-alert);">
         ...
         <?php if (!empty($emergency_notices)): ?>
                <div class="advisory-list">
                    <div class="advisory-title">EMERGENCY</div>
                    
                    <div class="advisory-card">
                        <?php foreach ($emergency_notices as $notice): ?>
                            <?php
                                // --- DATE CONVERSION LOGIC ---
                                $month_number = (int)$notice['month'];
                                $month_name = $notice['month']; // Fallback: use the number if conversion fails
                                
                                // Convert month number (e.g., 9) to month name (e.g., September)
                                // 'n' is the month format for 1 through 12
                                $date_obj = DateTime::createFromFormat('n', $month_number);
                                if ($date_obj) {
                                    $month_name = $date_obj->format('F'); // 'F' gives the full month name
                                }

                                // Combine month name, day, and year for display
                                $date_display = htmlspecialchars($month_name) . ' ' . htmlspecialchars($notice['day']) . ', ' . htmlspecialchars($notice['year']);
                                
                                // Use 'description' for the list item text
                                $text_display = htmlspecialchars($notice['description']); 
                            ?>
                            
                            <a href="#" class="advisory-list-item emergency">
                                <strong><?php echo $date_display; ?>:</strong> <?php echo $text_display; ?>
                            </a>
                        <?php endforeach; ?>
                    </div>
                </div>
                ...
                <?php if (!empty($scheduled_notices)): ?>
                <div class="advisory-list">
                    <div class="advisory-title scheduled">SCHEDULED</div>
                    
                    <div class="advisory-card scheduled">
                        <?php foreach ($scheduled_notices as $notice): ?>
                            <?php
                                // --- DATE CONVERSION LOGIC ---
                                $month_number = (int)$notice['month'];
                                $month_name = $notice['month']; // Fallback: use the number if conversion fails
                                
                                // Convert month number (e.g., 10) to month name (e.g., October)
                                $date_obj = DateTime::createFromFormat('n', $month_number);
                                if ($date_obj) {
                                    $month_name = $date_obj->format('F'); // 'F' gives the full month name
                                }
                            ...
                            <a href="#" class="advisory-list-item">
                                <strong><?php echo $date_display; ?>:</strong> <?php echo $text_display; ?>
                            </a>
                        <?php endforeach; ?>
                    </div>
                ...
                <?php if (empty($emergency_notices) && empty($scheduled_notices)): ?>
                    <p class="no-notices">There are no water interruption advisories at this time.</p>
                <?php endif; ?>
