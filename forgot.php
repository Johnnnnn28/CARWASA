<?php
// Start session for state management (OTP, phone, step, user_id)
session_start();

// *** REQUIRED: Include the Twilio PHP SDK ***
// This line assumes you installed Twilio via Composer.
...
// Credentials provided by the user (ACf088928d7bf3cfe7cd909b43ed49dfc2, c3b5f5b64873cc3d5631755b62c260c3)
define('TWILIO_ACCOUNT_SID', 'ACf088928d7bf3cfe7cd909b43ed49dfc2'); 
define('TWILIO_AUTH_TOKEN', 'c3b5f5b64873cc3d5631755b62c260c3'); 
// !!! CRITICAL: Replace this placeholder with your actual Twilio phone number (e.g., +15017122661)
...
// Phone number for display in Step 2, using session key 'fp_phone'
$phone_display = isset($_SESSION['fp_phone']) ? htmlspecialchars($_SESSION['fp_phone']) : '';
...
// Function to send OTP via Twilio SMS API
function sendTwilioOtp($phone, $otp) {
    try {
        ...
        // Mobile number must be in E.164 format (e.g., +639171234567)
        $messageBody = "Your CARWASA verification code is: " . $otp . ". It expires in 5 minutes.";

        $client->messages->create(
            $phone, // To: user's phone number
            [
        ...
        // Log the detailed error from Twilio
...
if ($action === 'identify_account' && $current_step === 1) {
    ...
    if (!$conn) {
        ...
        // 2. Query DB using email to fetch phone number and user ID
        // IMPORTANT: Assuming 'password_hash' column exists for update in Step 3
        $stmt = $conn->prepare("SELECT user_id, phone FROM users WHERE email = ?");
        ...
        if ($result->num_rows === 1) {
            ...
            $db_phone_number = trim($user['phone']); // Phone number from DB (e.g., 09123456789)
            
            // --- CRITICAL FIX: E.164 CONVERSION FOR PHILIPPINE NUMBERS ---
            $country_code = "+63"; // Philippines Country Code

            if (empty($db_phone_number)) {
                $error_message = "Recovery is not possible for this account. No phone number is registered for SMS.";
                $stmt->close();
                $conn->close();
                exit();
            }

            // Check if the number is in local format (09...) and convert to E.164 (+639...)
            if (substr($db_phone_number, 0, 1) === '0' && strlen($db_phone_number) === 11) {
                // Remove leading '0' and prefix with +63
                $phone_number = $country_code . substr($db_phone_number, 1);
            } else if (substr($db_phone_number, 0, 1) === '+' && strlen($db_phone_number) > 10) {
                 // Number is already in international format (e.g., +639171234567)
                $phone_number = $db_phone_number;
            } else {
                // Handle non-standard or unexpected formats
                $error_message = "Invalid phone number format in the database for recovery.";
                $stmt->close();
                $conn->close();
                exit();
            }
            ...
            // We store the original number ($db_phone_number) for display purposes in step 2.
            $_SESSION['fp_phone'] = $db_phone_number; 
            $_SESSION['fp_user_id'] = $user_id;
            $_SESSION['fp_otp'] = $otp; 
            $_SESSION['fp_otp_expiry'] = $expiry_time;

            // Send OTP via Twilio using the E.164 formatted $phone_number
            if (sendTwilioOtp($phone_number, $otp)) {
                 // Update session state for next step
                $_SESSION['fp_step'] = 2; 
                header("Location: forgot.php"); 
                exit();
            } else {
                // SMS failed, provide clear error message (not the vague one)
                $error_message = "OTP could not be sent. Please check the Twilio configuration or try again later.";
            }
            
        } else {
            // User NOT found. Display the direct error message as requested.
    ...
    if (empty($new_pass) || empty($confirm_pass)) {
        ...
        if (!$conn) {
            ...
            if ($stmt->execute()) {
                ...
                $_SESSION['fp_step'] = 4;
                unset($_SESSION['fp_phone'], $_SESSION['fp_user_id'], $_SESSION['fp_otp'], $_SESSION['fp_otp_expiry']);
                header("Location: forgot.php");
                exit();
            } else {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: var(--font-main); }
    ...
    .resend-link { text-align: right; margin-top: -10px; margin-bottom: 20px;}
...
<body>
    ...
    <div class="fp-card">
        ...
        <div class="header-section">
            <!-- Ensure logo.png is correctly placed or use an absolute path/fallback -->
        ...
        <div id="step1" class="step-content">
            <form method="POST" action="forgot.php">
                ...
                <div class="step-header-text">
                    ...
                    <p>Please enter the **Email Address** associated with your account to receive a One-Time Password (OTP) via SMS.</p>
                </div>
        ...
        <div id="step2" class="step-content">
            <div class="step-header-text">
                ...
                <p>Your OTP has been sent to the phone number: **<?php echo $phone_display; ?>**.<br>Please enter the 6-digit code below to verify.</p>
            </div>
            ...
            <form method="POST" action="forgot.php">
                ...
                <div class="resend-link">
                    <!-- Note: Resend link resets the process back to step 1 -->
                    <a href="forgot.php?resend=1">Resend OTP</a>
                </div>
